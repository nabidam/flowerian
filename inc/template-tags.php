	...
	function understrap_posted_on()
	{
		...
		$posted_on   = apply_filters(
			...
			sprintf(
				...
				esc_html_x('ارسال شده در تاریخ', 'تاریخ مطلب', 'understrap'),
		...
		$byline      = apply_filters(
			...
			sprintf(
				...
				$posted_on ? esc_html_x('توسط', 'نویسنده مطلب', 'understrap') : esc_html_x('ارسال شده توسط', 'نویسنده مطلب', 'understrap'),
...
if (!function_exists('understrap_entry_footer')) {
	...
	function understrap_entry_footer()
	{
		...
		if (!is_single() && !post_password_required() && (comments_open() || get_comments_number())) {
			...
			comments_popup_link(esc_html__('نظری بگذارید', 'understrap'), esc_html__('1 Comment', 'understrap'), esc_html__('% Comments', 'understrap'));
			echo '</span>';
		}
		edit_post_link(
			sprintf(
